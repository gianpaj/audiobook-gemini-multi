...
import type { ParsedStory, Segment } from "./types.js";
	...
	for (let i = 0; i < lines.length; i++) {
		...
		if (parsed) {
			...
			if (continuationMatch && currentSpeaker) {
				currentText.push(
					opts.trimText ? continuationMatch[1].trim() : continuationMatch[1],
				);
			}
...
export function getStorySummary(story: ParsedStory): string {
	...
	lines.push(
		`Speakers (${story.speakers.length}): ${story.speakers.join(", ")}`,
	);
	...
	for (const speaker of story.speakers) {
		...
		lines.push(
			`  ${speaker}: ${count} segments, ${chars.toLocaleString()} characters`,
		);
	}
...
export function filterBySpeaker(
	...
	return {
		...
		totalCharacters: filteredSegments.reduce(
