...
describe("parser", () => {
	describe("detectFormat", () => {
		...
		it("should return unknown for content without speaker tags", () => {
			expect(detectFormat("Just some plain text without speakers.")).toBe(
				"unknown",
			);
		});
	});
		...
		describe("bracket format", () => {
			it("should parse bracket format story correctly", () => {
				...
				result.segments.forEach((segment, index) => {
					expect(segment.speaker).toBe(
						EXPECTED_BRACKET_SEGMENTS[index].speaker,
					);
	...
	describe("validateParsedStory", () => {
		...
		it("should warn about very short segments", () => {
			...
			expect(
				validation.warnings.some((w) => w.includes("very short segments")),
			).toBe(true);
		});
		...
		it("should warn about very long segments", () => {
			...
			expect(
				validation.warnings.some((w) => w.includes("very long segments")),
			).toBe(true);
		});
	});
	...
	describe("filterBySpeaker", () => {
		...
		it("should update total characters", () => {
			...
			const expectedChars = filtered.segments.reduce(
	...
	describe("getSegmentRange", () => {
		...
		it("should update total characters for range", () => {
			...
			const expectedChars = range.segments.reduce(
				(sum, s) => sum + s.text.length,
				0,
			);
